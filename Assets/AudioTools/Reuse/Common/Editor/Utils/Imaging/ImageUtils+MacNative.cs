...
namespace ImpossibleRobert.Common
{
    ...
    /// Falls back to the registered optional imaging backend on any failure.
    /// </summary>
    public static partial class ImageUtils
    {
        ...
        internal static bool TryResizeImageNative(string originalFile, string outputFile, int maxSize, bool scaleBeyondSize, string typeOverride)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    Debug.LogWarning($"Native Mac resize failed for '{originalFile}', falling back to optional imaging backend: {e.Message}");
                }
                return false;
            }
            finally
        ...
        internal static bool TryComputePerceptualHashNative(string filePath, out ulong hash, int hashSize = 8)
        {
            ...
            catch (Exception e)
            {
                if (LogImageOperations)
                {
                    Debug.LogWarning($"Native Mac hash computation failed for '{filePath}', falling back to optional imaging backend: {e.Message}");
                }
                return false;
            }
            finally
        ...
        internal static bool TryComputePerceptualHashFromBytesNative(byte[] pngData, out ulong hash, int hashSize = 8)
        {
            ...
            catch (Exception e)
            {
                if (LogImageOperations)
                {
                    Debug.LogWarning($"Native Mac hash from bytes failed, falling back to optional imaging backend: {e.Message}");
                }
                return false;
            }
            finally
