...
namespace ImpossibleRobert.Common
{
    ...
    public static partial class ImageUtils
    {
        ...
        public static bool HasDominantColor(Texture2D texture, Color target, float marginPercent = 0.02f, float coverageThreshold = 0.3f)
        {
            if (texture == null) return false;

            Texture2D readable = texture.MakeReadable();
            try
            {
                Color32[] pixels = readable.GetPixels32();
                int matchCount = 0;
                byte targetR = (byte)Mathf.RoundToInt(target.r * 255f);
                byte targetG = (byte)Mathf.RoundToInt(target.g * 255f);
                byte targetB = (byte)Mathf.RoundToInt(target.b * 255f);
                ...
                return matchCount > pixels.Length * coverageThreshold;
            }
            finally
            {
                UnityEngine.Object.DestroyImmediate(readable);
        ...
        public static ulong ComputePerceptualHash(Texture2D texture, int hashSize = 8)
        {
            if (texture == null) return 0UL;

            Texture2D readable = texture.MakeReadable();
            try
            {
                Texture2D resized = readable.Downscale(hashSize, hashSize);
                try
                {
                    ...
                    for (int i = 0; i < pixels.Length; i++)
                    {
                        ...
                        double value = (color.r * 0.299) + (color.g * 0.587) + (color.b * 0.114);
                        luminance[i] = value;
                        sum += value;
                    }

                    double average = sum / luminance.Length;
                    ulong hash = 0UL;
        ...
        public static bool AreSimilar(Texture2D texture, ulong hashB, double maxFractionDifferent = 0.15)
        {
            ulong hashA = ComputePerceptualHash(texture);
        ...
        public static bool HasVisibleContent(this Texture2D texture, Color backgroundColor, out float detectionPercentage)
        {
            ...
            if (differentPercentage > threshold) return true;

            // Sparse lines or bone visualizations can fall between the sampled grid points
            // or occupy less than 0.3% of the frame. Run an exact fallback before declaring
            // the render empty so valid thin content is not discarded.
            Color[] pixels = texture.GetPixels();
            int exactDifferentPixels = 0;
            for (int i = 0; i < pixels.Length; i++)
            {
            ...
            float exactDifferentPercentage = pixels.Length > 0 ? (float)exactDifferentPixels / pixels.Length : 0f;
            if (exactDifferentPercentage > differentPercentage)
            {
                detectionPercentage = exactDifferentPercentage * 100f;
            }

            const float sparseThreshold = 0.001f; // 0.1% exact coverage for sparse but real content
            int minimumSparsePixels = Mathf.Max(4, Mathf.CeilToInt(pixels.Length * sparseThreshold));
            return exactDifferentPixels >= minimumSparsePixels;
        }
...
#else
                if (TryGetRegisteredImageDimensions(path, out int registeredWidth, out int registeredHeight))
                {
                    return Tuple.Create(registeredWidth, registeredHeight);
                }

                if (!ignoreErrors && LogImageOperations)
                {
                    Debug.LogWarning($"Could not determine image dimensions for '{file}': no optional imaging backend is registered.");
                }
                return null;
#endif
            }
